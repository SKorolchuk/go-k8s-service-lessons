...
func (s Store) QueryRolesByID(ctx context.Context, claims auth.Claims, userID string) (Roles, error) {
	if err := validate.CheckID(userID); err != nil {
	...
	var roles Roles
	if err := database.NamedQueryStruct(ctx, s.log, s.db, query, data, &roles); err != nil {
		if err == database.ErrNotFound {
			return Roles{}, database.ErrNotFound
		}
		return Roles{}, fmt.Errorf("selecting roles for user %q", data.UserID)
	}
