...
// QueryRolesByID gets roles of the specified user from the database.
func (c Core) QueryRolesByID(ctx context.Context, claims auth.Claims, userID string) (user.Roles, error) {

	// PERFORM PRE BUSINESS OPERATIONS

	roles, err := c.user.QueryRolesByID(ctx, claims, userID)
	if err != nil {
		return user.Roles{}, fmt.Errorf("query: %w", err)
	}
